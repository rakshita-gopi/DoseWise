...
export function RemindersPage() {
  ...
  const [loading, setLoading] = useState(!isCaregiver);

  const stockNotifications = notifications.filter((n) => STOCK_ALERT_TYPES.includes(n.type));

  useEffect(() => {
    if (!activePatient || isCaregiver) return;
    setLoading(true);
    inventoryApi
      .reminders(activePatient._id)
      .then(({ data }) => setDoses(data))
      .finally(() => setLoading(false));
  }, [activePatient, isCaregiver]);
  ...
  if (!activePatient && !isCaregiver) {
    return <EmptyState icon={<Bell />} title="No profile selected" />;
  }
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6 duration-500">
      <PageHeader
        title={isCaregiver ? 'Stock Alerts' : 'Reminders & Notifications'}
        description={
          isCaregiver
            ? 'SMS and in-app alerts when medicines run low or out of stock for people you care for'
            : `Today's doses and alerts for ${activePatient?.name}`
        }
      />
      ...
      <Card>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="section-title flex items-center gap-2">
            {isCaregiver && <AlertTriangle className="h-4 w-4 text-amber-500" />}
            {isCaregiver ? 'Stock Notifications' : 'Notifications'}
          </h2>
          <Button size="sm" variant="ghost" onClick={() => notificationApi.markAllRead().then(refreshNotifications)}>
            Mark all read
          </Button>
        </div>
        {(isCaregiver ? stockNotifications : notifications).length === 0 ? (
          <p className="muted py-6 text-center text-sm">
            {isCaregiver ? 'No stock alerts yet' : 'No notifications yet'}
          </p>
        ) : (
          <div className="space-y-2">
            {(isCaregiver ? stockNotifications : notifications).map((n: Notification) => (
              <div
                key={n._id}
              ...
              >
                <div>
                  ...
                  <p className="muted text-xs leading-relaxed">{n.message}</p>
